...
def run_experiment():
    ...
    chaos_texts = [
        "Glitch corrupted data error noise random.",
        "Chaos entropy disorder madness broken.",
        "System failure critical core dump hex binary.",
        "Zalgo text void scream static white noise.",
        "Meaningless patterns absurd dadaism random.",
    ]
    order_texts = [
        "Structure logic order purity perfection.",
        "System stable clear distinct organized.",
        "Mathematics crystal geometry symmetry.",
        "Rule law sequence pattern defined.",
        "Clean minimal calibrated synchronized.",
    ]
    ...
    # --- TEST CASES ---
    query_text = "Bitte verfasse ein kurzes Gedicht über den Frühling."
    memory_text = "System: Normaler Betriebsmodus." # Neutral anchor
    ...
    def steer_and_gen(coeff, label):
        ...
        output = generate_with_stored_kv(
            ...
            max_new_tokens=100, do_sample=True, temperature=0.9 # High temp for chaos
        )
    ...
    # 2. Mild Chaos
    logger.info("--- MILD CHAOS (2.0) ---")
    steer_and_gen(2.0, "Mild Chaos")

    # 3. High Chaos
    logger.info("--- HIGH CHAOS (5.0) ---")
    steer_and_gen(5.0, "High Chaos")

    # 4. EXTREME Chaos
    logger.info("--- EXTREME CHAOS (10.0) ---")
    steer_and_gen(10.0, "EXTREME Chaos")
    
    # 5. NEGATIVE Chaos (Hyper-Order)
    logger.info("--- HYPER ORDER (-5.0) ---")
    steer_and_gen(-5.0, "Hyper Order")
